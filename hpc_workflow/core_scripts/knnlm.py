...
class KNNWrapper(object):
    ...
    model_layer_to_capture = {
        ...
        'Olmo': {   # you can use lowercase or uppercase; we’ll normalize later
            KEY_TYPE.last_ffn_input: (
            lambda model: model.model.transformer.layers[-1].mlp,
            True
            ),
        KEY_TYPE.last_ffn_output: (
            lambda model: model.model.transformer.layers[-1],
            False
        ),
        },
        'olmo': {   # alias for safety
             KEY_TYPE.last_ffn_input: (
             lambda model: model.model.transformer.layers[-1].mlp,
            True
         ),
        ...
        'phi': {
            KEY_TYPE.last_ffn_input: (
            lambda model: model.model.layers[-1].mlp,   # FEED-FORWARD BLOCK
            True                                         # capture input
        ),
        KEY_TYPE.last_ffn_output: (
            lambda model: model.model.layers[-1],        # the full decoder layer
            False                                        # capture output
        ),
        },
        "phi3": {   # *** REQUIRED FOR Phi-3.5 + Mediphi ***
            KEY_TYPE.last_ffn_input: (
            lambda model: model.model.layers[-1].mlp,
            True
        ),
        ...
        "gemma2": {   # HF uses this for some ckpts
            KEY_TYPE.last_ffn_input: (
                lambda model: model.model.layers[-1].mlp,
                True
            ),
        ...
        "gemma3": {   # GEMMA-3 Instruct + Base models
            KEY_TYPE.last_ffn_input: (
                lambda model: model.model.layers[-1].mlp,
                True
            ),
        ...
        "qwen2": {
            KEY_TYPE.last_ffn_input: (
            lambda model: model.model.layers[-1].mlp,   # MLP block
            True                                        # capture MLP input
        ),
        KEY_TYPE.last_ffn_output: (
            lambda model: model.model.layers[-1],       # full decoder layer
            False                                       # capture layer output
        ),
        },
