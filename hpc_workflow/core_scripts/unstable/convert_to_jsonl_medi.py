...
def convert_csv_to_jsonl(input_file, output_file, one_shot_index=42, alternate_index=100, finetune=False):
    ...
    if finetune:
        for row in all_rows:
            ...
            section_text = (row.get(' Note') or '').strip()
            ...
            conversation = {
                ...
                "metadata": {
                    "icd10": row.get('ICD10', ''),
                    "icd10_desc": row.get('ICD10_desc', ''),  # kept for analysis only
                }
            }
    ...
    # -------- One-shot inference format (ICL) --------
        ...
        one_shot_answer = (one_shot_example.get(' Note') or '').strip()

        alternate_dialogue = (alternate_example.get('Dialogue') or '').strip()
        alternate_answer = (alternate_example.get(' Note') or '').strip()
        ...
        for idx, row in enumerate(all_rows):
            ...
            row_answer = (row.get(' Note') or '').strip()
