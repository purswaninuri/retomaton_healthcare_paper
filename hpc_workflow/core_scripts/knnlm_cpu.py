...
class KNNSaver(object):
    ...
    def post_forward_hook(self, module, input, output):
        ...
        # Align sequence length between keys and labels
        if shift == 1:
            captured_keys = captured_keys[:, :-shift]

        # Flatten to (batch*time, dim)
        captured_keys = captured_keys.flatten(0, 1)
        captured_values = self.labels[:, shift:].flatten(0, 1)

        # Align devices for consistent indexing
        device = captured_keys.device
        captured_values = captured_values.to(device)
        nonpad_mask = (captured_values != -100).to(device)

        # Index safely (now all on the same device)
        keys = captured_keys[nonpad_mask].detach().to('cpu')
        values = captured_values[nonpad_mask].detach().to('cpu')

        batch_time_size = keys.shape[0]

        # Prevent overflow if dataset smaller than declared
        ...
        try:
            self.dstore_keys[self.dstore_idx:self.dstore_idx + batch_time_size] = (
                keys.numpy().astype(np.float16)
            )
            self.dstore_vals[self.dstore_idx:self.dstore_idx + batch_time_size] = (
                values.unsqueeze(-1).numpy().astype(np.int32)
            )
        except ValueError as ex:
            logger.error(
                f"Error saving datastore with mode {self.dstore_keys.mode}, "
                f"did you try to save an already existing datastore?"
            )
            logger.error(
                f"Delete the files {self.dstore_keys.filename} and "
                f"{self.dstore_vals.filename} and try again"
            )
            raise ex
