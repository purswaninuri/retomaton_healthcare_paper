...
from knnlm_cpu import KNNWrapper, KNNSaver, KEY_TYPE, DIST
...
class DataTrainingArguments:
    ...
    metadata={
        "help": "Fraction of training data to use (e.g. 0.1 for 10%). "
                "Validation data remains unaffected."
    },
    )   
...
def main():
    ...
    # 🧠 Force CPU mode to ensure all datastore ops run on CPU
    device = torch.device("cpu")
    model.to(device)
    if training_args.fp16:
        logger.warning("Disabling fp16 since training is forced to CPU.")
        training_args.fp16 = False
    logger.info("⚙️ Running all computations on CPU for datastore consistency.")
    ...
    # ✅ Ensure train_dataset exists even if we're not training
    if not training_args.do_train and data_args.eval_subset == 'train':
        train_dataset = lm_datasets["train"]
    ...
    if data_args.train_fraction < 1.0:
        ...
        subset_size = int(total_size * data_args.train_fraction)
        np.random.seed(training_args.seed)
        indices = np.random.choice(total_size, subset_size, replace=False)
        train_dataset = train_dataset.select(indices.tolist())
        logger.info(f"⚠️ Using only {subset_size}/{total_size} ({data_args.train_fraction*100:.1f}%) of training data.")
