...
class DataTrainingArguments:
    ...
    apply_chat_template: Optional[bool] = field(
        default=False, metadata={"help": "Whether it's a conversation format or not."}
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # if extension == 'jsonl':
        #     raw_datasets = {}
        #     train_dataset = Dataset.from_list(load_jsonl(data_args.train_file))
        #     raw_datasets['train'] = train_dataset
        #     if data_args.validation_file:
        #         eval_dataset = Dataset.from_list(load_jsonl(data_args.validation_file))
        #         raw_datasets['validation'] = eval_dataset
        # else:
        raw_datasets = load_dataset(extension if extension != 'jsonl' else 'json', data_files=data_files, cache_dir=model_args.cache_dir, **dataset_args)
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            if data_args.apply_chat_template:
                # print([e for e in examples['messages']][:2])
                # print(tokenizer.chat_template)
                # for e in examples['messages']:
                #     print(e)
                #     print(tokenizer.apply_chat_template(e))
                #     print('-----')
                by_line_output = [tokenizer.apply_chat_template(e, return_dict=True) for e in examples['messages']]
                print(by_line_output)
                output = {c: [] for c  in by_line_output[0].keys()}
                for line in by_line_output:
                    for k in output.keys():
                        output[k].append(line[k])
            else:
    ...
    if data_args.block_size is None:
        ...
        if block_size > 8192:
            logger.warning(
                ...
                "Picking 8192 instead. You can change that default value by passing --block_size xxx."
            )
            block_size = 8192
    else:
