...
# Patch GPU-only FAISS functions for Colab (faiss-cpu)
if not hasattr(faiss, "GpuClonerOptions"):
    class DummyGC:
        def __init__(self): pass
    faiss.GpuClonerOptions = DummyGC

if not hasattr(faiss, "StandardGpuResources"):
    class DummyRes:
        def __init__(self): pass
    faiss.StandardGpuResources = DummyRes

if not hasattr(faiss, "index_cpu_to_gpu"):
    def dummy_index_cpu_to_gpu(*args, **kwargs):
        return args[2]   # return original CPU index
    faiss.index_cpu_to_gpu = dummy_index_cpu_to_gpu
...
retomaton = RetomatonWrapper(
    ...
    knn_gpu=False,#Changed this to False for Colab
    probe=32,
    k=args.k,
